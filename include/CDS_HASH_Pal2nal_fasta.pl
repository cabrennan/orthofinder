...
#    my %data = ();
    for (my $i = 0; $i < @c_files; $i++) {
        print "perl pal2nal.pl $a_files[$i] $c_files[$i] -output fasta -nogap -codontable 1 > $c_files[$i]_align.fa\n";
        system "perl pal2nal.pl $a_files[$i] $c_files[$i] -output fasta -nogap -codontable 1 > $c_files[$i]_align.fa\n";
